// Vercel serverless function — próximos 7 dias + eventos do mês selecionado
// Calendarios configurados via variables de entorno
...
// Calendarios ICS
const ICS_CALENDARS = [
  process.env.CAL_ICS_1 && {
    name: process.env.CAL_NAME_1 || 'Calendar 1',
    url: process.env.CAL_ICS_1
  },
...
function parseICSDate(str) {
  ...
  var y  = str.slice(0,4);
  var mo = str.slice(4,6);
  var d  = str.slice(6,8);
  var h  = str.slice(9,11);
  var mi = str.slice(11,13);
  var s  = str.slice(13,15);

  return new Date(
    y+'-'+mo+'-'+d+'T'+h+':'+mi+':'+s+
    (str.endsWith('Z') ? 'Z' : '-03:00')
  );
}


function parseICS(icsText, calName, minDateStr, maxDateStr) {
        ...
        if (startDate) {
          ...
          if (
            ...
            events.push({
              ...
              title:
                current.summary || '(sin título)',

              date:
                startStr,

              dayLabel:
                DIAS_ES[startDate.getDay()],
              ...
              sortKey:
                ...
                (isAllDay
                  ? 'T00:00'
                  : startDate.toISOString().slice(10)
                )
            });
          }
        }
